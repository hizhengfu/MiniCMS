...
if ($mc_get_type == 'post') {
    require __COMMON_PATH__.'/posts/index/publish.php';

    if (array_key_exists($mc_get_name, $mc_posts)) {
        $mc_post_id = $mc_get_name;
        $mc_post = $mc_posts[$mc_post_id];

        $mc_data = unserialize(file_get_contents(__COMMON_PATH__.'/posts/data/' . $mc_post_id . '.dat'));
    } else {
        mc_404();
    }
...
if ($mc_get_type != 'rss')
    require __COMMON_PATH__.'/theme/index.php';
else
    require __COMMON_PATH__.'/mc-rss.php';
?>
