...
if (!is_dir(__COMMON_PATH__.'/pages/data/'))
    mkdir(__COMMON_PATH__.'/pages/data/');
...
function load_pages()
{
    ...
    if (isset($_GET['state'])) {
        if ($_GET['state'] == 'draft') {
            $state = 'draft';
            $index_file = __COMMON_PATH__.'/pages/index/draft.php';
        } else if ($_GET['state'] == 'delete') {
            $state = 'delete';
            $index_file = __COMMON_PATH__.'/pages/index/delete.php';
        } else {
            $state = 'publish';
            $index_file = __COMMON_PATH__.'/pages/index/publish.php';
        }
    } else {
        $state = 'publish';
        $index_file = __COMMON_PATH__.'/pages/index/publish.php';
...
function delete_page($id)
{
    ...
    if ($state != 'delete') {
        $index_file2 = __COMMON_PATH__.'/pages/index/delete.php';
        ...
        unlink(__COMMON_PATH__.'/pages/data/' . $page['file'] . '.dat');
    }
}
...
function revert_page($id)
{
    ...
    $index_file2 = __COMMON_PATH__.'/pages/index/' . $prev_state . '.php';
...
<div class="table_list_tool">
  ...
  <span class="pager">
    ...
    <a class="link_button"
       href="?state=<?php echo $state; ?>&date=<?php echo $filter_date; ?>&page=<?php echo $prev_page; ?>">&lsaquo;</a>
    第 <input type="text" value="<?php echo $page_num; ?>" id="page_input_1"/> 页,共 <?php echo $last_page; ?> 页
    <a class="link_button"
       href="?state=<?php echo $state; ?>&date=<?php echo $filter_date; ?>&page=<?php echo $next_page; ?>">&rsaquo;</a>
    <a class="link_button"
       href="?state=<?php echo $state; ?>&date=<?php echo $filter_date; ?>&page=<?php echo $last_page; ?>">&raquo;</a>
  </span>
    <script type="text/javascript">
    ...
    <table colspan="0" rowspan="0" cellpadding="0" cellspacing="0" id="list">
        ...
        <?php for ($i = 0; $i < $page_count; $i++) {
            ...
            <tr<?php if ($i % 2 == 0) echo ' class="alt"'; ?>>
                ...
                <td>
                    <a class="row_name"
                       href="page-edit.php?file=<?php echo $page['file']; ?>"><?php echo htmlspecialchars($page['title']);?></a>

                    <div class="row_tool">
                        <a class="link_button" href="page-edit.php?file=<?php echo $page['file']; ?>">编辑</a>
                        <?php if ($state == 'delete') { ?>
                            <a class="link_button"
                               href="?revert=<?php echo $page_id; ?>&state=<?php echo $state; ?>&date=<?php echo $filter_date; ?>">还原</a>
                            <a class="link_button"
                               href="?delete=<?php echo $page_id; ?>&state=<?php echo $state; ?>&date=<?php echo $filter_date; ?>">删除</a>
                        <?php } else { ?>
                            <a class="link_button"
