...
if (isset($_POST['_IS_POST_BACK_'])) {
    $page_file = isset($_POST['file']) ? $_POST['file'] : '';
    $page_path = isset($_POST['path']) ? $_POST['path'] : '';
    $page_state = isset($_POST['state']) ? $_POST['state'] : '';
    $page_title = isset($_POST['title']) ? trim($_POST['title']) : '';
    $page_content = isset($_POST['content']) ? trim($_POST['content']) : '';
    $page_content = get_magic_quotes_gpc() ? stripslashes(trim($page_content)) : trim($page_content);
    $page_date = date("Y-m-d");
    $page_time = date("H:i:s");
    $page_can_comment = isset($_POST['can_comment']) ? $_POST['can_comment'] : '';
    ...
    if ($page_title == '') {
        ...
        if ($page_file == '') {
            ...
            foreach ($file_names as $file_name) {
                $file_path = __COMMON_PATH__ . '/pages/data/' . $file_name . '.dat';
            ...
            $file_path = __COMMON_PATH__ . '/pages/data/' . $page_file . '.dat';
            ...
            if ($page_old_state != $page_state || $page_old_path != $page_path) {
                $index_file = __COMMON_PATH__ . '/pages/index/' . $page_old_state . '.php';
        ...
        $index_file = __COMMON_PATH__ . '/pages/index/' . $page_state . '.php';
    ...
    $file_path = __COMMON_PATH__ . '/pages/data/' . $_GET['file'] . '.dat';
...
<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post">
    ...
    <?php if ($succeed) { ?>
        <?php if ($page_state == 'publish') { ?>
            <div class="updated">页面已发布。 <a href=../?"<?php echo $page_path; ?>/"
        ...
        <div style="float:left">
            ...
            <select name="year">
                ...
                <?php $year = substr($page_date, 0, 4); for ($i = 1990; $i <= 2030; $i++) { ?>
                    <option
                        value="<?php echo $i; ?>" <?php if ($year == $i) echo 'selected="selected";' ?>><?php echo $i; ?></option>
                <?php } ?>
            </select> -
            <select name="month">
                <option value=""></option>
                <?php $month = substr($page_date, 5, 2); for ($i = 1; $i <= 12; $i++) {
                    $m = sprintf("%02d", $i); ?>
                    <option
                        value="<?php echo $m; ?>" <?php if ($month == $m) echo 'selected="selected";' ?>><?php echo $m; ?></option>
                <?php } ?>
            </select> -
            <select name="day">
                <option value=""></option>
                <?php $day = substr($page_date, 8, 2); for ($i = 1; $i <= 31; $i++) {
                    $m = sprintf("%02d", $i); ?>
                    <option
                        value="<?php echo $m; ?>" <?php if ($day == $m) echo 'selected="selected";' ?>><?php echo $m; ?></option>
                <?php } ?>
            </select>&nbsp;
            <select name="hourse">
                <option value=""></option>
                <?php $hourse = substr($page_time, 0, 2); for ($i = 0; $i <= 23; $i++) {
                    $m = sprintf("%02d", $i); ?>
                    <option
                        value="<?php echo $m; ?>" <?php if ($hourse == $m) echo 'selected="selected";' ?>><?php echo $m; ?></option>
                <?php } ?>
            </select> :
            <select name="minute">
                <option value=""></option>
                <?php $minute = substr($page_time, 3, 2); for ($i = 0; $i <= 59; $i++) {
                    $m = sprintf("%02d", $i); ?>
                    <option
                        value="<?php echo $m; ?>" <?php if ($minute == $m) echo 'selected="selected";' ?>><?php echo $m; ?></option>
                <?php } ?>
            </select> :
            <select name="second">
                <option value=""></option>
                <?php $second = substr($page_time, 6, 2); for ($i = 0; $i <= 59; $i++) {
                    $m = sprintf("%02d", $i); ?>
                    <option
                        value="<?php echo $m; ?>" <?php if ($second == $m) echo 'selected="selected";' ?>><?php echo $m; ?></option>
                <?php } ?>
            </select>
        </div>
        <?php
        if (!empty($mc_config['comment_code'])) {
            ?>
            评论：
