...
if (isset($_POST['_IS_POST_BACK_'])) {
    ...
    $post_content = get_magic_quotes_gpc() ? stripslashes(trim($post_content)) : trim($post_content);
    ...
    if ($post_title == '') {
        ...
        if ($post_id == '') {
            ...
            foreach ($file_names as $file_name) {
                $file_path = __COMMON_PATH__ . '/posts/data/' . $file_name . '.dat';
            ...
            $file_path = __COMMON_PATH__ . '/posts/data/' . $post_id . '.dat';
            ...
            if ($post_old_state != $post_state) {
                $index_file = __COMMON_PATH__ . '/posts/index/' . $post_old_state . '.php';
        ...
        $index_file = __COMMON_PATH__ . '/posts/index/' . $post_state . '.php';
    ...
    $file_path = __COMMON_PATH__ . '/posts/data/' . $_GET['id'] . '.dat';
...
<form action="<?php echo $_SERVER['REQUEST_URI']; ?>" method="post">
    ...
    <div style="margin-bottom:20px;text-align:right">
        <div style="float:left">
            ...
            <select name="month">
                <option value=""></option>
                <?php $month = substr($post_date, 5, 2); for ($i = 1; $i <= 12; $i++) {
                    $m = sprintf("%02d", $i); ?>
                    <option
            ...
            <select name="day">
                <option value=""></option>
                <?php $day = substr($post_date, 8, 2); for ($i = 1; $i <= 31; $i++) {
                    $m = sprintf("%02d", $i); ?>
                    <option
            ...
            <select name="hourse">
                <option value=""></option>
                <?php $hourse = substr($post_time, 0, 2); for ($i = 0; $i <= 23; $i++) {
                    $m = sprintf("%02d", $i); ?>
                    <option
            ...
            <select name="minute">
                <option value=""></option>
                <?php $minute = substr($post_time, 3, 2); for ($i = 0; $i <= 59; $i++) {
                    $m = sprintf("%02d", $i); ?>
                    <option
            ...
            <select name="second">
                <option value=""></option>
                <?php $second = substr($post_time, 6, 2); for ($i = 0; $i <= 59; $i++) {
                    $m = sprintf("%02d", $i); ?>
                    <option
